...
// ============================================================
const CRED_KEY = "jpsoft_tienda_cred";
...
function initFirebase() {
  ...
  _unsubs.push(onSnapshot(collection(db, "productos"), snap => {
    ...
    renderProductosTabla();
    updateStockBadge();
    if (typeof renderHistorialPrecios === "function") renderHistorialPrecios();
  }));
  ...
  _unsubs.push(onSnapshot(doc(db, "config", "backup"), snap => {
    ...
    if (el) {
      if (snap.exists() && snap.data().ultimoBackup) el.textContent = "Último backup: " + snap.data().ultimoBackup;
      else el.textContent = "No hay backups registrados.";
    }
    if (typeof verificarBackup === "function") verificarBackup(snap.exists() ? (snap.data().ultimoBackup || "") : "");
  }));
...
//  NAVEGACIÓN
// ============================================================
const VIEWS = { venta: "Venta", caja: "Caja", productos: "Productos", proveedores: "Proveedores", reportes: "Reportes", "historial-precios": "Historial de precios", usuarios: "Usuarios", backup: "Backup" };
...
// ── Helpers para exportar Excel desde Caja ──
...
function exportarExcelCaja(hojas, filename) {
...
// Exportar ventas del día/período
document.getElementById("btnExportarVentasExcel")?.addEventListener("click", () => {
  const desde = cajaFechaKey;
  const hasta = cajaFechaKey;
  const keys  = dateRange(desde, hasta);
  const data  = [["Fecha","Turno","Hora","Producto","Cantidad","P. Unitario","Subtotal","Método","Total Venta"]];
  ...
  keys.forEach(k => {
    ...
    ["manana","tarde"].forEach(turno => {
      ...
      Object.values(t.ventas).forEach(v => {
        (v.items||[]).forEach(i => {
          data.push([
            k, turno === "manana" ? "Mañana" : "Tarde",
            v.hora||"", i.desc||"", i.qty||0,
  ...
  if (data.length === 1) { showToast("No hay ventas para exportar.", "warning"); return; }
  exportarExcelCaja([{ nombre: "Ventas", data, colsMoney: [5,6,8] }], `JPSoft_Tienda_Ventas_${desde}.xlsx`);
  showToast("Ventas exportadas ✓", "success");
});

// Exportar caja del día
document.getElementById("btnExportarCajaExcel")?.addEventListener("click", () => {
  const desde = cajaFechaKey;
  const data  = [["Fecha","Turno","Apertura","Cierre","Fondo Inicial","Ventas","Efectivo","Mercado Pago","Débito","Crédito","Total"]];

  const dia = cajaData[desde];
  if (!dia) { showToast("No hay datos de caja para este día.", "warning"); return; }

  ["manana","tarde"].forEach(turno => {
    const t = dia[turno];
    if (!t?.apertura) return;
    const ventas = t.ventas ? Object.values(t.ventas) : [];
    let totE = 0, totM = 0, totD = 0, totC = 0;
    ventas.forEach(v => {
      if (v.metodo === "efectivo") totE += v.total||0;
      else if (v.metodo === "mp") totM += v.total||0;
      else if (v.metodo === "debito") totD += v.total||0;
    ...
    data.push([
      desde, turno === "manana" ? "Mañana" : "Tarde",
      t.apertura.hora||"", t.cierre?.hora||"—",
      t.apertura.fondo||0, ventas.length,
      totE, totM, totD, totC, totE+totM+totD+totC
    ]);
  });

  if (data.length === 1) { showToast("No hay datos de caja para exportar.", "warning"); return; }
  exportarExcelCaja([{ nombre: "Caja", data, colsMoney: [4,6,7,8,9,10] }], `JPSoft_Tienda_Caja_${desde}.xlsx`);
  showToast("Caja exportada ✓", "success");
});
...
Esta acción no se puede deshacer.`)) return;
  ...
  const cajaActual = { ...cajaHoy };
  const turnoData  = { ...cajaActual[turno] };
  const ventasActuales = { ...(turnoData.ventas || {}) };
  delete ventasActuales[ventaId];
  turnoData.ventas = ventasActuales;
  setDoc(doc(db, 'caja', fecha), { [turno]: turnoData }, { merge: true });
    ...
    return `<tr data-id="${p._id}">
      ...
      <td>
        <div style="display:flex;gap:5px;justify-content:flex-end">
          ...
          <button class="btn-danger" style="font-size:11px;padding:4px 7px" data-delete onclick="window._eliminarProducto('${p._id}','${(p.desc||'').replace(/'/g,'&#39;')}')">🗑</button>
        </div>
      </td>
    </tr>`;
  }).join("");
...
// ── Eliminar producto directo desde la tabla ──
...
// ── Exportar lista de precios como Excel ──
document.getElementById("btnExportarListaExcel")?.addEventListener("click", () => {
  const filtro = document.getElementById("prodFilterProv")?.value || "";
  ...
  if (!lista.length) { showToast("No hay productos para exportar.", "error"); return; }
  ...
  XLSX.writeFile(wb, `JPSoft_Tienda_Precios_${todayKey()}.xlsx`);
  showToast("Lista exportada ✓", "success");
});

// ── Imprimir lista de precios como PDF ──
document.getElementById("btnImprimirListaPrecios")?.addEventListener("click", async () => {
  const filtro = document.getElementById("prodFilterProv")?.value || "";
  const lista  = allProducts
    .filter(p => !filtro || p.proveedor === filtro)
    .sort((a,b) => (a.proveedor||"").localeCompare(b.proveedor||"") || (a.desc||"").localeCompare(b.desc||""));

  if (!lista.length) { showToast("No hay productos para imprimir.", "error"); return; }
  ...
  const rows = lista.map(p => {
    ...
    return `<tr>
      <td>${p.proveedor||"—"}</td>
      <td>${p.id||"—"}</td>
      <td style="font-family:monospace;font-size:10px">${p.cod||"—"}</td>
      <td style="font-weight:500">${p.desc||"—"}</td>
      <td style="text-align:right">${fmt(p.lista||0)}</td>
      <td style="text-align:center">${ganPct}</td>
      <td style="text-align:right;font-weight:600">${venta}</td>
    </tr>`;
  }).join("");
  ...
  const content = `
    <div style="font-family:'DM Sans',sans-serif;font-size:11px;color:#111;padding:8mm 10mm 8mm 10mm;width:210mm;box-sizing:border-box">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;border-bottom:2px solid #111;padding-bottom:6px">
        <div style="font-size:14px;font-weight:600">JPSoft | Tienda — Lista de Precios</div>
        <div style="font-size:10px;color:#888">${lista.length} productos · ${now}</div>
      </div>
      ${seccionesPDF}
      <div style="font-size:9px;color:#bbb;text-align:center;margin-top:10px;border-top:1px solid #eee;padding-top:6px">JPSoft | Tienda · ${now}</div>
    </div>`;
  ...
  try {
    ...
    pdf.save(`JPSoft_Tienda_Precios_${todayKey()}.pdf`);
...
function renderProveedores() {
  ...
  grid.innerHTML = lista.map(([id, p]) => {
    ...
    return `<div class="proveedor-card" data-id="${id}">
...
Esta acción no se puede deshacer.`;
  ...
  showToast(`Proveedor "${nombre}" y sus productos eliminados ✓`, "success");
};
...
//  FIREBASE
// ============================================================
// initFirebase y rebuildGananciaMap ya definidos arriba — duplicados eliminados
