...
import {
  ...
  onSnapshot, deleteDoc, updateDoc, query, orderBy, serverTimestamp,
...
async function checkConnection() {
  try {
    await fetch("https://www.gstatic.com/firebasejs/10.12.0/firebase-app.js", {
...
async function confirmarVentaFinal() {
  if (!estaOnline) {
    showToast("Sin conexión — no se puede confirmar la venta.", "error");
    return;
  }
  ...
  if (Object.keys(stockUpdates).length) {
    ...
    for (const [path, val] of Object.entries(stockUpdates)) {
      const prodId = path.split('/')[1];
      await updateDoc(doc(db, 'productos', prodId), { stock: val });
    }
  }
  ...
  // Leer turno actual de Firestore y agregar la venta
  const cajaRef  = doc(db, 'caja', todayKey());
  const cajaSnap = await getDoc(cajaRef);
  const cajaActual = cajaSnap.exists() ? cajaSnap.data() : {};
...
function renderTurnoCard(turnoKey, turno, esHoy, manana, tarde) {
  ...
  if (yaCerrado) {
    ...
    btnReabrir.addEventListener("click", async () => {
      ...
      const _snapR = await getDoc(doc(db, 'caja', cajaFechaKey));
      const _dataR = _snapR.exists() ? _snapR.data() : {};
      const _turnoR = _dataR[turnoKey] || {};
      delete _turnoR.cierre;
      await setDoc(doc(db, 'caja', cajaFechaKey), { [turnoKey]: _turnoR }, { merge: true });
