...
async function confirmarVentaFinal() {
  ...
  await push(ref(db, `caja/${todayKey()}/${_turno}/ventas`), {
    hora,
...
function renderTurnoCard(turnoKey, turno, esHoy, manana, tarde) {
  ...
  if (!yaAbierto && esHoy && puedeAbrir) {
    // Formulario apertura
    card.innerHTML = `
      <div style="font-size:14px;font-weight:600;margin-bottom:12px">Abrir Turno ${label}</div>
      <div class="form-group">
        <label class="form-label">Fondo inicial ($)</label>
        <input type="number" class="form-input" id="fondoInput_${turnoKey}" placeholder="Ej: 5000" min="0" />
      </div>
      <button class="btn-primary" id="btnAbrir_${turnoKey}" style="width:100%;justify-content:center">
        Abrir Turno ${label}
      </button>`;
    ...
    setTimeout(() => {
      document.getElementById(`btnAbrir_${turnoKey}`)?.addEventListener("click", async () => {
        ...
        await set(ref(db, `caja/${cajaFechaKey}/${turnoKey}/apertura`), {
          hora: nowHora(), fondo, turno: label, admin: getNombreUsuario()
        });
  ...
  if (!yaAbierto) {
    // Turno no abierto y no se puede abrir aún
    card.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div style="font-size:14px;font-weight:600;color:var(--text3)">Turno ${label}</div>
        <span class="badge badge-neutral">Sin abrir</span>
      </div>`;
    return card;
  }
  ...
  if (yaCerrado) {
    ...
    btnReabrir.addEventListener("click", async () => {
      ...
      await remove(ref(db, `caja/${cajaFechaKey}/${turnoKey}/cierre`));
...
document.getElementById("btnConfirmarCierre").addEventListener("click", async () => {
  ...
  await set(ref(db, `caja/${cajaFechaKey}/${cierreTurnoActivo}/cierre`), {
    hora:  nowHora(),
    admin: getNombreUsuario()
  });
