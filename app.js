...
document.getElementById("btnImprimirCierre")?.addEventListener("click", () => {
  ...
  document.getElementById("printArea").innerHTML = `
    <div class="print-brand">JPSoft | QBV</div>
    <div class="print-sub">Resumen de cierre de caja</div>

    <div class="print-section">Información del día</div>
    <div class="print-row"><span class="lbl">Fecha</span><span class="val">${fechaLabel}</span></div>
    <div class="print-row"><span class="lbl">Turno</span><span class="val">${apertura.turno || "—"}</span></div>
    <div class="print-row"><span class="lbl">Apertura</span><span class="val">${fmtHora(apertura.hora) || "—"}</span></div>
    <div class="print-row"><span class="lbl">Cierre</span><span class="val">${fmtHora(cierre.hora) || "—"}</span></div>
    <div class="print-row"><span class="lbl">Fondo inicial</span><span class="val">${apertura.fondo ? fmt(apertura.fondo) : "—"}</span></div>
    <div class="print-row"><span class="lbl">Responsable</span><span class="val">${apertura.admin || "—"}</span></div>

    <hr class="print-divider" />

    <div class="print-section">Resumen por método de pago</div>
    <div class="print-row"><span class="lbl" style="color:#1a7a50;font-weight:500">Efectivo</span><span class="val">${fmt(totE)}</span></div>
    <div class="print-row"><span class="lbl" style="color:#185fa5;font-weight:500">Débito</span><span class="val">${fmt(totD)}</span></div>
    <div class="print-row"><span class="lbl" style="color:#009ee3;font-weight:500">Mercado Pago</span><span class="val">${fmt(totM)}</span></div>
    <div class="print-total"><span>Total recaudado</span><span>${fmt(totTotal)}</span></div>

    <hr class="print-divider" />

    <div class="print-section">Detalle de ventas (${ventas.length})</div>
    <table>
      <thead><tr><th>Hora</th><th>Descripción</th><th>Método</th><th style="text-align:right">Total</th></tr></thead>
      <tbody>${ventasRows}</tbody>
    </table>

    <div class="print-footer">Generado el ${now} · JPSoft | QBV</div>`;
