...
function renderGastos() {
  ...
  tbody.innerHTML = ordenados.map(g => {
    ...
    return `<tr>
      ...
      <td>
        <div style="display:flex;gap:5px;justify-content:flex-end">
          <button class="btn-secondary" style="font-size:11px;padding:4px 8px" data-gasto-edit="${g._id}" data-fecha="${gastoFechaKey}">Editar</button>
          <button class="btn-danger" style="font-size:11px;padding:4px 7px;opacity:.8" data-gasto-anular="${g._id}" data-fecha="${gastoFechaKey}" data-desc="${(g.desc || g.cat).replace(/"/g,'&quot;')}" data-monto="${g.monto || 0}">✕ Anular</button>
        </div>
      </td>
    </tr>`;
  }).join("");
