  ...
  if (detalle) {
    if (!ventas.length) {
      ...
      detalle.innerHTML = [...ventas].sort((a,b) => b.fecha.localeCompare(a.fecha) || (b.hora||"").localeCompare(a.hora||"")).map(v => `
        <div class="timeline-row" style="grid-template-columns:100px 1fr 110px 60px 90px">
          <span style="font-family:'DM Mono',monospace;font-size:11px;color:var(--text3);white-space:nowrap">${fechaLabel(v.fecha)}</span>
          <span style="font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(v.items||[]).map(i=>i.desc).join(", ")}</span>
          <span class="${metClass[v.metodo]||""}">${metLabel[v.metodo]||v.metodo}</span>
          <span class="num">${(v.items||[]).reduce((s,i)=>s+i.qty,0)}</span>
          <span class="num" style="font-weight:500">${fmt(v.total)}</span>
...
document.getElementById("btnExportarReportePDF")?.addEventListener("click", async () => {
  ...
  const content = `<div style="font-family:'DM Sans',sans-serif;font-size:12px;color:#111;padding:8mm 10mm;width:210mm;box-sizing:border-box">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;border-bottom:2px solid #111;padding-bottom:6px">
      <div style="font-size:16px;font-weight:600">JPSoft | Tienda — Reportes</div>
      <div style="font-size:10px;color:#888">${periodoLabel} · ${ventas.length} ventas</div>
    </div>
    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin-bottom:14px">
      ${[["Total",fmt(tot),"#111"],["Efectivo",fmt(totE),"#1a7a50"],["Mercado Pago",fmt(totM),"#009ee3"],["Débito",fmt(totD),"#185fa5"],["Crédito",fmt(totC),"#185fa5"]].map(([l,v,col])=>`<div style="border:1px solid #eee;border-radius:5px;padding:8px"><div style="font-size:9px;color:#aaa;text-transform:uppercase;margin-bottom:3px">${l}</div><div style="font-size:13px;font-weight:600;color:${col}">${v}</div></div>`).join("")}
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
      <div>
        <div style="font-size:9px;font-weight:600;color:#aaa;text-transform:uppercase;margin-bottom:6px">Resumen por día</div>
        <table style="width:100%;border-collapse:collapse;font-size:10px">
          <thead><tr style="background:#f5f5f5"><th style="text-align:left;padding:4px 6px">Fecha</th><th style="text-align:right;padding:4px 6px">Ventas</th><th style="text-align:right;padding:4px 6px">Total</th><th style="text-align:right;padding:4px 6px">Prom.</th></tr></thead>
          <tbody>${diasRows}</tbody>
        </table>
      </div>
      <div>
        <div style="font-size:9px;font-weight:600;color:#aaa;text-transform:uppercase;margin-bottom:6px">Productos más vendidos</div>
        <table style="width:100%;border-collapse:collapse;font-size:10px">
          <thead><tr style="background:#f5f5f5"><th style="text-align:left;padding:4px 6px">Producto</th><th style="text-align:right;padding:4px 6px">Unid.</th><th style="text-align:right;padding:4px 6px">Total</th></tr></thead>
          <tbody>${prodRows}</tbody>
        </table>
      </div>
    </div>
    <div style="font-size:9px;color:#bbb;text-align:center;margin-top:12px;border-top:1px solid #eee;padding-top:5px">JPSoft | Tienda · ${now}</div>
  </div>`;
  ...
  try {
    ...
    for (let i = 0; i < pages; i++) {
      ...
      pageCanvas.width = canvas.width; pageCanvas.height = pageH * 2;
