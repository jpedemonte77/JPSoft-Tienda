...
function renderCaja() {
  ...
  document.getElementById("btnImprimirCierre").classList.toggle("hidden", !yaHayCierre);
...
document.getElementById("btnImprimirCierre")?.addEventListener("click", () => {
  const caja   = cajaData[cajaFechaKey] || {};
  const apertura = caja.apertura || {};
  const cierre   = caja.cierre   || {};
  const ventas   = caja.ventas   ? Object.values(caja.ventas) : [];
  ...
  const totTotal = totE + totD + totM;

  const [y, m, d] = cajaFechaKey.split("-").map(Number);
  const fechaLabel = new Date(y, m-1, d).toLocaleDateString("es-AR", { weekday:"long", day:"2-digit", month:"2-digit", year:"numeric" });
  ...
  const ventasRows = [...ventas]
    ...
    .map(v => {
      ...
      const descCorto = desc.length > 35 ? desc.slice(0, 35) + "…" : desc;
      return `<tr>
  ...
  // Nombre del archivo: JPSoft_QBV_TM_01-06-2026 o JPSoft_QBV_TT_01-06-2026
  const turnoCode = (apertura.turno || "").toLowerCase().startsWith("m") ? "TM" : "TT";
  const [fy, fm, fd] = cajaFechaKey.split("-");
  const nombrePDF = `JPSoft_QBV_${turnoCode}_${parseInt(fd)}-${parseInt(fm)}-${fy}`;
  ...
  const printContent = `
    <div style="font-family:'DM Sans',sans-serif;font-size:13px;color:#111;padding:2rem;max-width:520px;margin:0 auto">
      ...
      <div style="font-size:12px;color:#888;margin-bottom:1.5rem">Resumen de cierre de caja</div>

      <div style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.08em;color:#aaa;margin-bottom:8px">Información del día</div>
      <div style="display:flex;justify-content:space-between;margin-bottom:5px"><span style="color:#555">Fecha</span><span style="font-weight:500">${fechaLabel}</span></div>
      <div style="display:flex;justify-content:space-between;margin-bottom:5px"><span style="color:#555">Turno</span><span style="font-weight:500">${apertura.turno || "—"}</span></div>
      <div style="display:flex;justify-content:space-between;margin-bottom:5px"><span style="color:#555">Apertura</span><span style="font-weight:500">${fmtHora(apertura.hora) || "—"}</span></div>
      <div style="display:flex;justify-content:space-between;margin-bottom:5px"><span style="color:#555">Cierre</span><span style="font-weight:500">${fmtHora(cierre.hora) || "—"}</span></div>
      <div style="display:flex;justify-content:space-between;margin-bottom:5px"><span style="color:#555">Fondo inicial</span><span style="font-weight:500">${apertura.fondo ? fmt(apertura.fondo) : "—"}</span></div>
      <div style="display:flex;justify-content:space-between;margin-bottom:5px"><span style="color:#555">Responsable</span><span style="font-weight:500">${apertura.admin || "—"}</span></div>

      <hr style="border:none;border-top:1px solid #eee;margin:12px 0" />

      <div style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.08em;color:#aaa;margin-bottom:8px">Resumen por método de pago</div>
      <div style="display:flex;justify-content:space-between;margin-bottom:5px"><span style="color:#1a7a50;font-weight:500">Efectivo</span><span>${fmt(totE)}</span></div>
      <div style="display:flex;justify-content:space-between;margin-bottom:5px"><span style="color:#185fa5;font-weight:500">Débito</span><span>${fmt(totD)}</span></div>
      <div style="display:flex;justify-content:space-between;margin-bottom:5px"><span style="color:#009ee3;font-weight:500">Mercado Pago</span><span>${fmt(totM)}</span></div>
      <div style="display:flex;justify-content:space-between;font-size:16px;font-weight:600;padding-top:10px;border-top:2px solid #111;margin-top:6px"><span>Total recaudado</span><span>${fmt(totTotal)}</span></div>

      <hr style="border:none;border-top:1px solid #eee;margin:12px 0" />

      <div style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.08em;color:#aaa;margin-bottom:8px">Detalle de ventas (${ventas.length})</div>
      <table style="width:100%;border-collapse:collapse;font-size:12px">
        <thead><tr>
          <th style="text-align:left;font-size:10px;color:#aaa;text-transform:uppercase;padding:4px 0;border-bottom:1px solid #eee">Hora</th>
          <th style="text-align:left;font-size:10px;color:#aaa;text-transform:uppercase;padding:4px 0;border-bottom:1px solid #eee">Descripción</th>
          <th style="text-align:left;font-size:10px;color:#aaa;text-transform:uppercase;padding:4px 0;border-bottom:1px solid #eee">Método</th>
          <th style="text-align:right;font-size:10px;color:#aaa;text-transform:uppercase;padding:4px 0;border-bottom:1px solid #eee">Total</th>
        </tr></thead>
