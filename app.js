...
const SECCIONES_AVANZADO   = ["inicio","venta","caja","gastos","productos","combos","proveedores","compras","clientes","presupuestos","notas","reportes","historial-precios","actividad","usuarios","soporte","backup"];
var modoActual        = "avanzado";
var opcionalesActivos = [];

function aplicarModo() {
  var visibles;
  if (modoActual === "basico") visibles = SECCIONES_FIJAS.slice();
  else if (modoActual === "avanzado") visibles = SECCIONES_AVANZADO.slice();
  else visibles = SECCIONES_AVANZADO.filter(function(s) {
    return SECCIONES_FIJAS.indexOf(s) >= 0 || opcionalesActivos.indexOf(s) >= 0;
  });
  var nav = document.querySelector(".sidebar-nav");
  if (!nav) return;
  nav.querySelectorAll("[data-view]").forEach(function(btn) { btn.style.display = "none"; });
  visibles.forEach(function(view) {
    var btn = nav.querySelector('[data-view="' + view + '"]');
    if (btn) { btn.style.display = ""; nav.appendChild(btn); }
  });
}

function carregarModoDesdeStorage() {
