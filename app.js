  ...
  if (ventas.length) {
    ...
    timelineWrap.innerHTML = `
      <div class="timeline-header">
        <span>Descripción</span><span class="num">Total</span><span>Método</span><span>Hora</span><span class="num">Ítems</span><span>Usuario</span>
      </div>
    ...
    return `<div class="proveedor-card" data-id="${id}">
      <div class="proveedor-card-header">
        <div style="display:flex;align-items:center;gap:8px">
          <input type="checkbox" class="prov-check" data-id="${id}" style="cursor:pointer;width:14px;height:14px;flex-shrink:0">
...
// ── Selección múltiple proveedores ──
document.getElementById("proveedoresGrid")?.addEventListener("change", e => {
  if (e.target.classList.contains("prov-check")) actualizarBarraSeleccionProv();
});

function actualizarBarraSeleccionProv() {
  const checks = document.querySelectorAll(".prov-check:checked");
  const bar    = document.getElementById("provSeleccionBar");
  const count  = document.getElementById("provSeleccionCount");
  if (checks.length > 0) {
    bar.style.display = "flex";
    count.textContent = checks.length + (checks.length === 1 ? " seleccionado" : " seleccionados");
  } else {
...
document.getElementById("btnEliminarProvSeleccionados")?.addEventListener("click", async () => {
  const checks = [...document.querySelectorAll(".prov-check:checked")];
  if (!checks.length) return;
  // Contar productos asociados
  const ids    = checks.map(cb => cb.dataset.id);
  const nombres = ids.map(id => proveedores[id]?.nombre).filter(Boolean);
  const totalProds = allProducts.filter(p => nombres.includes(p.proveedor)).length;
  const aviso = totalProds > 0
    ? `¿Eliminar ${checks.length} proveedor${checks.length > 1 ? "es" : ""}?
Se eliminarán también ${totalProds} producto${totalProds > 1 ? "s" : ""} asociado${totalProds > 1 ? "s" : ""}.
Esta acción no se puede deshacer.`
    : `¿Eliminar ${checks.length} proveedor${checks.length > 1 ? "es" : ""}?
Esta acción no se puede deshacer.`;
  if (!confirm(aviso)) return;
  ids.forEach(id => deleteDoc(doc(db, 'proveedores', id)));
  allProducts.filter(p => nombres.includes(p.proveedor)).forEach(p => deleteDoc(doc(db, 'productos', p._id)));
  showToast(`${checks.length} proveedor${checks.length > 1 ? "es eliminados" : " eliminado"} ✓`, "success");
});

document.getElementById("btnDeseleccionarProvTodos")?.addEventListener("click", () => {
  document.querySelectorAll(".prov-check").forEach(cb => cb.checked = false);
  actualizarBarraSeleccionProv();
});
