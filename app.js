...
function renderCart() {
  ...
  const descLabel    = document.getElementById("cartDescuentoLabel");
  const descMon      = document.getElementById("cartDescuentoMonto");
  const el    = document.getElementById("cartItems");
  const empty = document.getElementById("cartEmpty");

  if (cartCount)    cartCount.textContent = keys.length + (keys.length === 1 ? " ítem" : " ítems");
  if (cartSubtotal) cartSubtotal.textContent = fmtDec(subtotal);
  ...
  if (descRow) {
    if (descMonto > 0) {
      ...
      if (descLabel) descLabel.textContent = "Descuento" + sufijo;
      if (descMon)   descMon.textContent   = "− " + fmtDec(descMonto);
    } else {
...
document.getElementById("btnConfirmarVenta").addEventListener("click", () => {
  ...
  const descHtml = descMonto > 0
    ? `<div style="display:flex;justify-content:space-between;color:#c0391a;font-size:12px"><span>Descuento${descuentoTipo === "pct" ? ` (${descuentoValor}%)` : ""}</span><span>− ${fmtDec(descMonto)}</span></div>`
    : "";
