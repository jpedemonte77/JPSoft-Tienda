...
// ============================================================
let allProducts   = [];
let proveedores   = {};   // { id: { nombre, ganancia, tabaco, categoria } }
...
onAuthStateChanged(auth, user => {
  if (user) {
    ...
    const nombre = ADMIN_NOMBRES[user.email] || user.email.split("@")[0];
...
function rebuildGananciaMap() {
  ...
  Object.values(proveedores).forEach(p => {
    gananciaMap[p.nombre] = (p.ganancia || 0) / 100;
  });
}
  ...
  el.innerHTML = keys.map(k => {
    ...
    return `<div class="cart-item">
      ...
      <div class="cart-item-right">
        ...
        <div class="qty-wrap">
          <button class="qty-btn" onclick="window._changeQty('${k}',-1)">-</button>
          <span class="qty-val">${qty}</span>
          <button class="qty-btn" onclick="window._changeQty('${k}',1)">+</button>
        </div>
      </div>
      <button class="cart-del" onclick="window._removeFromCart('${k}')">×</button>
    </div>`;
  }).join("");
}
...
document.querySelectorAll(".pago-btn").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    document.querySelectorAll(".pago-btn").forEach(b => {
      b.className = "pago-btn" + (b.classList.contains("full") ? " full" : "");
    });
...
document.getElementById("btnConfirmarVenta").addEventListener("click", () => {
  ...
  const metodoLabel = { efectivo: "Efectivo 💵", debito: "Débito 💳", mp: "Transferencia / QR 📲" };
  document.getElementById("ventaModalSub").textContent = `${fecha} — ${hora} hs · ${metodoLabel[metodoSeleccionado]}`;
  ...
  keys.forEach(k => {
    ...
    rowsHtml += `<div style="display:flex;justify-content:space-between;gap:8px"><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#555">${det}</span><span style="font-weight:500;flex-shrink:0">${fmtDec(sub)}</span></div>`;
  });

  document.getElementById("ventaTicketPreview").innerHTML = `
    <div style="font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;margin-bottom:2px">JPSoft | QBV</div>
    <div style="font-size:11px;color:#999;margin-bottom:10px;font-family:'DM Sans',sans-serif">${fecha} — ${hora} hs · ${metodoLabel[metodoSeleccionado]}</div>
    ${rowsHtml}
    <hr style="border:none;border-top:1px dashed rgba(0,0,0,.15);margin:10px 0">
    <div style="display:flex;justify-content:space-between;font-family:'DM Sans',sans-serif;font-weight:600;font-size:14px"><span>TOTAL</span><span>${fmtDec(total)}</span></div>`;

  window._ventaPendiente = { keys, total, hora, fecha };
...
function renderCaja() {
  ...
  document.getElementById("cajaSubtitulo").textContent = apertura.turno + " · Apertura " + apertura.hora;
  ...
  document.getElementById("cajaEstadoBadge").innerHTML = yaHayCierre
    ? `<span class="badge badge-neutral">Cerrada ${cierre.hora}</span>`
  ...
  // Info apertura
  document.getElementById("infoAperturaHora").textContent  = apertura.hora || "—";
  ...
  if (!ventas.length) {
    ...
    const metLabel = { efectivo: "Efectivo", debito: "Débito", mp: "MP / QR" };
    ...
    timeline.innerHTML = [...ventas].reverse().map(v => `
      <div class="timeline-row">
        ...
        <span style="font-family:'DM Mono',monospace;font-size:12px;color:var(--text3)">${v.hora || "—"}</span>
  ...
  if (yaHayCierre) {
    document.getElementById("cierreSummaryContent").innerHTML = `
      <div style="font-size:13px;color:var(--text2);line-height:2">
        <div style="display:flex;justify-content:space-between"><span>Hora de cierre</span><span style="font-family:'DM Mono',monospace">${cierre.hora}</span></div>
...
function renderProductosTabla() {
  ...
  tbody.innerHTML = slice.map(p => {
    ...
    const stock   = p.stock ?? "—";
    const stockClass = typeof p.stock === "number"
      ? (p.stock <= 0 ? "badge-danger" : p.stock <= (p.stockMin || 5) ? "badge-warn" : "badge-success")
      : "badge-neutral";
    const tabaco = Object.values(proveedores).find(pr => pr.nombre === p.proveedor)?.tabaco;

    return `<tr>
      <td class="id-cell">${p.id || "—"}</td>
      <td><span class="badge ${badgeClass(p.proveedor)}">${p.proveedor || "—"}</span></td>
      <td class="cod-cell" style="font-size:10px">${p.cod || "—"}</td>
      <td style="font-size:13px;font-weight:500;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${p.desc || ""}">${p.desc || "—"}</td>
      <td class="cod-cell">${p.barcode || "—"}</td>
      <td class="num">${fmt(p.lista)}</td>
      <td class="num"><span class="badge ${tabaco ? "b-tabaco" : "badge-neutral"}">${gan}</span></td>
  ...
  grid.innerHTML = lista.map(([id, p]) => {
    ...
    return `<div class="proveedor-card">
      <div class="proveedor-card-header">
        ...
        ${p.tabaco ? `<span class="badge b-tabaco">🚬 Tabaco</span>` : `<span class="badge badge-neutral">${p.categoria || "General"}</span>`}
      </div>
      <div class="proveedor-row"><span class="label">Margen ganancia</span><span class="val"><span class="badge ${p.tabaco ? "b-tabaco" : "badge-neutral"}">${p.ganancia ?? 0}%</span></span></div>
...
function abrirModalProveedor(id) {
  ...
  document.getElementById("vf-tabaco").checked  = p?.tabaco    || false;
